...
select(Homo.sapiens, keys=k, cols=c(), keytype="ENTREZID")
@
...
The 1st thing that we do is set up a data frame that expresses the way
that these different packages relate to each other.  To do this, we
make an \Robject{data.frame} that contains 4 columns.  The 1st two
columns indicate the package names, and the 2nd two columns give the
keys that connect each row/pair of packages together.  Please note
that the key names in this \Robject{data.frame} are the same strings
that are returned by the \Rmethod{cols} method for the individual
packages.
...
<<TempDir, echo=FALSE, results=hide>>=
tmpout = tempdir()
@ 
...
file.rename(file.path(tmpout, "Homo.sapiens"),file.path(tmpout, "foo.db"))
@
