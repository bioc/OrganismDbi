...
<<transcripts>>=
transcripts(Homo.sapiens, cols=c("TXNAME","SYMBOL"))
@ 

And the \Rclass{GRanges} object that would be returned would have the
information that you specified in the cols argument.  You could also
...
<<transcriptsBy>>=
transcriptsBy(Homo.sapiens, by="gene", cols=c("TXNAME","SYMBOL"))
@ 
...
returned by the \Rmethod{cols} method for the individual packages.
